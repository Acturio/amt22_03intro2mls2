...
Al igual que muchas cosas en la vida, los vectores propios y los valores propios vienen en pares: cada vector propio tiene un valor propio correspondiente. En pocas palabras, **un vector propio es una dirección**, como "vertical" o "45 grados", mientras que un **valor propio es un número que indica cuánta variación hay en los datos en esa dirección**. 

::: {.infobox .pin data-latex="{pin}"}

* El vector propio con el valor propio más alto es, por lo tanto, el primer componente principal. Siendo el valor propio, la varianza de este.
...
Podemos reformular un conjunto de datos en términos de estos vectores propios y valores propios sin cambiar la información subyacente. Tomemos en cuenta que reformular un conjunto de datos con respecto a un conjunto de valores propios y vectores propios no implica cambiar los datos en sí, solo los estaremos observando desde un ángulo diferente, lo cual debería representar mejor los datos.
...
Podemos observar que en la primera componente principal, las $9$ variables que utilizó en Consejo Nacional de Población para obtener el [Índice de Marginación 2010](http://www.conapo.gob.mx/work/models/CONAPO/Resource/862/4/images/06_C_AGEB.pdf) aportan de manera positiva en el primer componente principal.
...
**¿Cómo están distribuídos los estados de la República en las primeras dos componentes principales?**
...
* *columns* Los nombres de las columnas que se imputarán y utilizarán para la imputación. 
