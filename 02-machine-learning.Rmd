...
```{r}
recipe_esp <- recipe(HHV ~ carbon + hydrogen + oxygen + nitrogen + sulfur, data = biomass_tr) %>% 
              step_impute_knn(carbon, nitrogen,
...
```{r}
recipe <- recipe(HHV ~ carbon + hydrogen + oxygen + nitrogen + sulfur,
                 data = biomass_tr) %>% 
...
plot(cbind( biomass%>%  filter( dataset == 'Testing') %>% select(nitrogen) , imputed[,4]))
