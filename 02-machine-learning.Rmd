...
Es común encontrar en la literatura este tema junto con la regresión múltiple, sin embargo, el alcance de esta transformación va a más allá de la regresión, por lo que preferimos manejar este tema como parte de la ingeniería de variables y no como un modelo per se.

El objetivo de la transformación polinomial dentro de la ingeniería de variables es crear nuevas variables que puedan explicar la relación entre variable de respuesta y variable explicativa a través de un polinomio de grado *k*.
...
set.seed(9039)
carb_missing <- sample(1:nrow(biomass_te_whole), 25)
nitro_missing <- sample(1:nrow(biomass_te_whole), 15)
...
imputed <- bake(recipe, biomass_te) # prueba con test
...
Los siguientes ejercicios serán clave en el éxito del los siguientes capítulos, por lo que se trabajará en equipo para lograr adquirir el mayor aprendizaje. 

1. Se crearán 3 o 4 equipos para enriquecer el aprendizaje y participación de todos

2. Cada equipo estará a cargo de desarrollar una receta de *feature engineering* utilizando los pasos vistos en el curso pasado y el actual.

3. Tendrán 15 días para probar distintas estrategias que mejoren las predicciones del precio de ventas.

4. Se deberá entregar y explicar el código creado por el equipo. Este código servirá para los ejercicios de optimización de modelos en los siguientes capítulos.
